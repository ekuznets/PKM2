...
void InitEncounters()
{
	...
	// small merchant
		...
		rEnc.Type = ENCOUNTER_TRADE;
		rEnc.MinRank = 1;
		Enc_AddShips(rEnc, "Merchant", 1, 5); // 3
	...
	// 
		...
		rEnc.Type = ENCOUNTER_TRADE;
		rEnc.MinRank = 3;
		Enc_AddShips(rEnc, "Merchant", 2, 5); //3
		Enc_AddShips(rEnc, "War", 1, 2); //1
		Enc_ExcludeNation(rEnc, PIRATE);
	...
	// 
		...
		rEnc.Type = ENCOUNTER_TRADE;
		rEnc.MinRank = 4;
		Enc_AddShips(rEnc, "Merchant", 1, 3); //1
		Enc_AddShips(rEnc, "War", 2, 5); //3
		Enc_ExcludeNation(rEnc, PIRATE);
