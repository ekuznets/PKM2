...
void InitInterface_I(string iniName,int bIState)
{
    ...
    // boal -->
    CreateString(true,"HullPoints","",FONT_NORMAL,COLOR_NORMAL,307,349,SCRIPT_ALIGN_LEFT,1.0);
    // boal <--
	// summary strings
...
void FillFourImages()
{
    ...
    for(i=0; i<4; i++)
    {
		...
		// boal GameInterface.FourImage.(tmpStr).str2		= XI_ConvertString(tmp2);
...
void SetNoneData()
{
	...
	if(shipType!=SHIP_NOTUSED)
	{
		...
		// boal -->
		GameInterface.strings.HullPoints = sti(refBaseShip.HP);
		// boal <--
	}
	else
	{
