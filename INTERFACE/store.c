...
// boal --> START MOD Code by Stone-D : 28/07/2003
    SetEventHandler("SellAllPress","SellAllProcess",0);
// boal <-- END MOD Code by Stone-D : 28/07/2003
...
// boal --> START MOD Code by Stone-D : 29/07/2003
void SellAllProcess()
{
	int c,i,m,p;
	int tQuantity;
	...
	for(i=0;i<GOODS_QUANTITY;i++)
	{
//		Uncomment this next line if you want to sell everything EXCEPT ammunition.
//		if( CheckAttribute(Goods[i],"DamageHull") ) { continue; }
//		Uncomment this next line if you want to sell everything EXCEPT repair goods.
//		if( i==GOOD_SAILCLOTH || i==GOOD_PLANKS ) { continue; }
		if( !GetStoreGoodsUsed(refStore,i) ) { continue; }
		...
		p = GetGoodPriceByType(i,tQuantity,GetStoreGoodsPrice(refStore,i,PRICE_TYPE_SELL,refCharacter));
		nPlayerMoney = nPlayerMoney + p;
		c = c + p;
		Log_SetStringToLog(GoodStr + ": " + tQuantity + " : " + p);
	}
	mainRef.Money = nPlayerMoney;
	Log_SetStringToLog(" Δενεγ: " + m + "+" + c + "=" + mainRef.Money);
	UpdateScroll();
	SetShowMode(SHOW_MAIN);
	RefreshAllStrings();
	SetBuySellEnable();
...
// boal <-- END MOD Code by Stone-D : 29/07/2003
