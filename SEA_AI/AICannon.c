...
float Cannon_GetRechargeTime()
{
	...
	// Ship.Crew.Quantity - скока народу сейчас
	// Ship.Crew.Morale - текущая мораль 0..99 45 - средняя
	// Ship.maxcrew
	// Ship.MinCrew
	// aCharacter.skill.Leadership - строка с числом
	float cQ  = GetCrewQuantity(aCharacter);
	float cMC = GetMaxCrewQuantity(aCharacter);
	float cCM;
	if(!CheckAttribute(aCharacter,"Ship.Crew.Morale"))
    {
        cCM = 45;
    }
    else
    {
	   cCM = sti(aCharacter.Ship.Crew.Morale);
    }
	if (cMC)
	{
