...
// boal -->
	...
	{*/
        ...
        //Log_SetStringToLog("ret "+iNewBallType+" "+ bBalls+" "+ bBombs+" "+ bGrapes+" "+ bKnippels);
//	}

//    Log_SetStringToLog("Char " + rCharacter.id + " distance " + fMinEnemyDistance + " Cannon "+ rCharacter.Ship.Cannons.Type);
//boal <--
	if (iNewBallType < 0)
	{
		if (!IsCompanion(rCharacter))
...
/*
exampl (test result)
rCannon.FireRange = 500
BALLS = 500 (1)
KNIPPELS = 350 (0.7) need to check sales status
BOMBS = 210   (0.42)
GRAPES = 80   (0.16) " CannonRange"+ rCannon.FireRange+
*/
//    Log_SetStringToLog("Char " + rCharacter.id + " distance " + fMinEnemyDistance + " Charge "+sti(rCharacter.Ship.Cannons.Charge.Type));
    if ( (fMinEnemyDistance > (range * 0.48)) && bBalls) return GOOD_BALLS;
    if ( (fMinEnemyDistance > (range * 0.42)) && bKnippels) return GOOD_KNIPPELS;
    if ( (fMinEnemyDistance > (range * 0.13)) && bBombs) return GOOD_BOMBS;
...
void CharacterUpdateShipFromBaseShip(int iCharacterIndex)
{
	...
	if (!CheckAttribute(rCharacter,"Ship.Cannons.Charge.Type")) { rCharacter.Ship.Cannons.Charge.Type = GOOD_BALLS; }// boal GOOD_BOMBS; }
...
void Ship_CreateForCharacter(ref rCharacter, int iShipType, string sShipName)
{
	...
	rCharacter.Ship.Cannons.Borts = "";
	rCharacter.Ship.Cannons.Charge.Type = GOOD_BALLS;// boal GOOD_BOMBS;
	rCharacter.Ship.Ang.y = 0.0;
